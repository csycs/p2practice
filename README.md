...
3. p2与egret的锚点位置不一样！
   ...
   - 必须一定要注意，p2的坐标是按照在中心的锚点计算的，egret的坐标是按照在左上角的锚点计算的。所以当你在输出一个 convex 图形的顶点时，以 box 为例，假设 box 的 length 为 100，height 为 60，你会发现四个顶点分别为: [-50, -30], [50, -30], [50, 30], [-50, 30]。
...
---
## 调试相关
- 用egret去渲染p2算法库时，目前发现有两种方法：
   1. 在egret的 EnterToFrame 或 Tricker 中去实时的更新贴图的位置角度等
   2. 在画出图形的时候，直接去将位置给到图形内部。
---